...
export const actions = {
  ...
  async getLatestArticles({ commit, state }, para) {
    ...
    if (res.result && res.result.length > 0) {
      let arr = await res.result.map(x => {
        x.updateAt = Coms.getCommonTime(x.updateAt);
  ...
  async getArticle({ commit, state }, para) {
    ...
    if (res.isSuc) {
      res.result.updateAt = Coms.getCommonTime(res.result.updateAt);
  ...
  async sign_out({ commit, state }) {
    ...
    if (res.isSuc) {
      ...
      Cookies.remove("_55lover_avatar"); // removed!
      Cookies.remove("_55lover_email"); // removed!
      Cookies.remove("_55lover_id"); // removed!
      Cookies.remove("_55lover_name"); // removed!
      $message(res.message);
    }
  }
  // // 获取动态列表
