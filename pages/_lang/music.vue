      ...
      <div class="flex-1 left-content">
        <div class="music">
          <div class="title flex flex-align-center">
            <span class="iconfont dp-icon-music dp-icon-00AACD"></span>
            <span class="title-text">音乐专栏</span>
            <span class="title-label">Music column</span>
          </div>
          <div class="music-header m-content bgbox flex flex-align-center">
            <img class="avatar" src="http://scenery.55lover.com/image/scenery/23115938.7a36240.jpg" alt="dengpeng">
            <div class="info flex-1">
              <div class="user"><span class="name">邓鹏</span> <span class="from">{{ $t('message.MusicUser') }}</span></div>
              <div class="intro">
                <span>{{ $t('message.PersonalIntroduction') }}</span> <span>{{ $t('message.PersonalIntroductionContent') }}</span>
              </div>
              <div class="address">
                <span>{{ $t('message.Region') }}</span> <span>{{ $t('message.RegionContent') }}</span>
              </div>
              <div class="age">
                <span>{{ $t('message.Age') }}</span> <span>{{ $t('message.AgeContent') }}</span>
              </div>
            </div>
          </div>
          <div class="music-list m-content bgbox">
            <div class="music-list-title">
              {{ $t('message.SongList') }} <span>{{ $t('message.SongsTotal') }}{{ list.length }}</span>
            </div>
            <ul class="list">
...
export default {
  async asyncData({ app }) {
    ...
    let res = await app.$axios.$post(api.audio.getAudioList, para);
    if (res.isSuc) {
      var list = res.result,
  ...
  methods: {
    ...
    async getQQMusic() {
      // 自定义配置axios
      // var axiosin = await this.$axios.create({
